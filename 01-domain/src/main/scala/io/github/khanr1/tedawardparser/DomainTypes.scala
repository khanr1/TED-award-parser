...
opaque type ContractingBodyName = String
object ContractingBodyName:
  def apply(s: String): ContractingBodyName = s

  given show: Show[ContractingBodyName] = Show.show(n => n.value)

  extension (d: ContractingBodyName) def value: String = d
...
object NoticeType:
  private val noticaTypeLookUP: Map[NoticeType, String] = Map(
    ...
    CN -> "contract notice",
    CM -> "modification of a contract/concession during its term"
  )
...
object ProcurementProcess:
  private val procProsLookup: Map[ProcurementProcess, String] = Map(
    ...
    CompetitiveProcedure -> "competitive procedure with negotiation",
    NotApplicable -> "not applicable",
    Unknown -> "unknown"
  )
  given show: Show[ProcurementProcess] = Show.show(proc => procProsLookup(proc))

  def toDomain(s: String) =
    procProsLookup.map((k, v) => (v, k)).getOrElse(s.toLowerCase(), Unknown)
...
object Country {
  ...
  val fullNames: Map[Country, String] = Map(
    AF -> "Afghanistan",
    AX -> "Åland Islands",
    AL -> "Albania",
    DZ -> "Algeria",
  ...
  given Show[Country] with {
    def show(c: Country): String =
      fullNames.getOrElse(c, c.toString)
  }

  val codeMap = fullNames.map(c => c._1.toString() -> c._1).toMap

  def toDomain(s: String): Country =

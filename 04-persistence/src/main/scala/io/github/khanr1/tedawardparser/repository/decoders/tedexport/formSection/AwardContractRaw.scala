...
private def emptyAward(reason: String): AwardContractRaw = AwardContractRaw(
  ...
  contractorNationalID =
    Left(ParserError.MissingField("Contractor NationalID", None)),
  contractorAddress =
    Left(ParserError.MissingField("Contractor Address", None)),
  contractorTown = Left(ParserError.MissingField("Contractor Town", None)),
  contractorPostalCode =
    Left(ParserError.MissingField("Contractor PostalCode", None)),
  contractorCountry = None,
  contractorPointOfContact =
    Left(ParserError.MissingField("Contractor PointOfContact", None)),
  ...
  given XMLDecoder[AwardContractRaw] =
    new XMLDecoder[AwardContractRaw]:
      override def decode(e: Elem): AwardContractRaw =
        if e.child.collect { case el: scala.xml.Elem => el }.isEmpty then
          return emptyAward("No awarded contract")
        if (e \ "NO_AWARDED_CONTRACT").nonEmpty then
          return emptyAward("No awarded contract")

        val path = FormSectionPathR208.F03.AwardContractPath

        val currency =
          e.attrAt(path.contractValueCurrencyPath attr ("CURRENCY"))
        ...
        AwardContractRaw(
          contractNumber =
            e.textAtOrError(path.contractNumberPath, "Contract Number"),
          lotNumber = e.textAtOrError(path.lotNumberPath, "Lot Number"),
          contractTitle =
            e.textAtOrError(path.contractTitlePath, "Contract Title"),
          awardDate = e.textAtOrError(path.awardDatePath, "Award Date"),
          contractorName =
            e.textAtOrError(path.contractorNamePath, "Contractor Name"),
          contractorNationalID = e.textAtOrError(
            path.contractorNationalIDPath,
            "Contractor NationalID"
          ),
          contractorAddress =
            e.textAtOrError(path.contractorAddressPath, "Contractor Address"),
          contractorTown =
            e.textAtOrError(path.contractorTownPath, "Contractor Town"),
          contractorPostalCode = e.textAtOrError(
            path.contractorPostalCodePath,
            "Contractor PostalCode"
          ),
          contractorCountry = e.attrAt(path.contractorCountryPath),
          contractorPointOfContact = e.textAtOrError(
            path.contractorPointOfContactPath,
            "Contractor PointOfContact"
          ),
          contractorPhone =
            e.textAtOrError(path.contractorPhonePath, "Contractor Phone"),
          contractorEmail =
            e.textAtOrError(path.contractorEmailPath, "Contractor Email"),
  ...
  given XMLDecoder[AwardContractRaw] =
    new XMLDecoder[AwardContractRaw]:
      override def decode(e: Elem): AwardContractRaw =
        ...
        val value = e
          .textAtOrError(path.contractValueAmountPath, "Amount")
        ...
        val base1 =
          awarded / "CONTRACTORS" / "CONTRACTOR" / "ADDRESS_CONTRACTOR"
        ...
        AwardContractRaw(
          ...
          contractTitle =
            e.textAtOrError(path.contractTitlePath, "Contract Title"),
          awardDate = e.textAtOrError(path.awardDatePath, "Award Date"),
          contractorName = e.firstTextOrError(
            List(base1 / "OFFICIALNAME", base2 / "OFFICIALNAME"),
            "Contractor Name"
          ),
          contractorNationalID = e.firstTextOrError(
            List(base1 / "NATIONALID", base2 / "NATIONALID"),
            "Contractor NationalID"
          ),
          contractorAddress = e.firstTextOrError(
            List(base1 / "ADDRESS", base2 / "ADDRESS"),
            "Contractor Address"
          ),
          contractorTown = e.firstTextOrError(
            List(base1 / "TOWN", base2 / "TOWN"),
            "Contractor Town"
          ),
          contractorPostalCode = e.firstTextOrError(
            List(base1 / "POSTAL_CODE", base2 / "POSTAL_CODE"),
            "Contractor PostalCode"
          ),
          contractorCountry = e.firstAttr(
            List(base1 / "COUNTRY" attr "VALUE", base2 / "COUNTRY" attr "VALUE")
          ),
          contractorPointOfContact = e.firstTextOrError(
            List(base1 / "CONTACT_POINT", base2 / "CONTACT_POINT"),
            "Contractor PointOfContact"
          ),
          contractorPhone = e.firstTextOrError(
            List(base1 / "PHONE", base2 / "PHONE"),
            "Contractor Phone"
          ),
          contractorEmail = e.firstTextOrError(
            List(base1 / "E_MAIL", base2 / "E_MAIL"),
            "Contractor Email"
          ),
          contractValue = contractValue
        )

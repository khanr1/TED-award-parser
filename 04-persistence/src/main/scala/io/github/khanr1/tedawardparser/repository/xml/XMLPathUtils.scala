...
object XMLPathUtils:
  ...
  extension (e: Elem)
    ...
    def nodesAt(path: XMLPath, ns: Ns = Ns.empty): NodeSeq = {
      ...
      val walked: NodeSeq =
        walkSegments.zipWithIndex.foldLeft(NodeSeq.fromSeq(Seq(e: Node))) {
          case (candidates, (Segment.Elem(q), idx)) =>
          ...
          case (candidates, (_: Segment.Attr, _)) =>

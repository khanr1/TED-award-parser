...
object NoticeMapper:
  ...
  private def opt(e: Either[ParserError, String]): Option[String] = e.toOption.filter(_.nonEmpty)
  ...
  private def mapCodedData(p: ParsedNotice): CodedDataSection =
    ...
    CodedDataSection(
      officialJournalReference = OfficialJournalReference(
        journalSeries   = OfficialJournalSeries(str(ojr.journalSerie)).getOrElse(OfficialJournalSeries.S),
        journalNumber   = OfficialJournalNumber(str(ojr.journalNumber)).getOrElse(OfficialJournalNumber("0").getOrElse(throw new RuntimeException("impossible"))),
  ...
  private def mapObjectContract(raw: ContractAwardObjectInformationRaw): ObjectContract =
    ObjectContract(
    ...
    AwardContract(
      ...
      contractor     = opt(raw.contractorName).map(name =>
        Contractor(
          ...
          pointOfContact = PointOfContact(opt(raw.contractorPointOfContact).getOrElse("")),
  ...
  private def mapComplementaryInfo(raw: ComplementaryInformationRaw): ComplementaryInformation =
    ComplementaryInformation(
      additionalInformation = raw.additionalInformation.toOption.filter(_.nonEmpty).map(AdditionalInformation.apply),
      relatesToEuProject    = raw.relatesToEuProject.filter(_.nonEmpty).map(EuProjectReference.apply),
      noticeDispatchDate    = raw.noticeDispatchDate.toOption.filter(_.nonEmpty).map(DispatchDate.apply),
      appealBodyName        = raw.appealBodyName.toOption.filter(_.nonEmpty).map(Name.apply),
      justification         = raw.justification.toOption.filter(_.nonEmpty).map(Justification.apply)
    )
  ...
  private def mapContractAward(p: ParsedNotice): ContractAward =
    ContractAward(
      ...
      complementaryInformartion = mapComplementaryInfo(p.complementaryInformation)
    )
